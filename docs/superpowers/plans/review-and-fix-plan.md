...
import { ProductCreateSchema, ProductUpdateSchema, type Product } from "@repo/tenant-orm/schemas/tenant";
...
export function ProductForm({ initialData, onSubmit, onCancel }: ProductFormProps) {
  const form = useForm({
    resolver: zodResolver(initialData ? ProductUpdateSchema : ProductCreateSchema),
...
export function ProductDrawer({ open, onOpenChange, initialData, onSubmit, title }: ProductDrawerProps) {
  return (
