...
from src.services.stripe_adapter import CheckoutItem
...
async def create_storefront_checkout_session(
    ...
    result = await adapter.create_checkout(
        ...
        items=items,
        success_url=f"{request.base_url}{tenant_slug}/checkout/success?session_id={{CHECKOUT_SESSION_ID}}",
        cancel_url=f"{request.base_url}{tenant_slug}/checkout",
        db=db,
    )
