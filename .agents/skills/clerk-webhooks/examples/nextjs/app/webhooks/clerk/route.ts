...
export async function POST(request: Request) {
  try {
    ...
    switch (event.type) {
      case "user.created":
        console.log("New user created:", {
          ...
          email: (
            event.data as { email_addresses?: Array<{ email_address: string }> }
    ...
    if (message.includes("Missing required Svix headers"))
      errorResponse = "Missing required Svix headers";
    else if (message === "No matching signature found")
      errorResponse = "Invalid signature";
    else if (message === "Message timestamp too old")
      errorResponse = "Timestamp too old";
